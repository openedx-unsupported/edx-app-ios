    ...
    func setup() {
        ...
        actionButton.oex_addAction({[weak self] _ in
            ...
            self?.action?( {[weak self] _ in
                            ...
                            self?.environment.analytics.enrollCourseEvent(courseId: self?.courseCard.course?.course_id ?? "", name: AnalyticsEventName.CourseEnrollmentClicked.rawValue, displayName: AnalyticsDisplayName.EnrolledCourseClicked.rawValue)
            } )
            }, for: .touchUpInside)
