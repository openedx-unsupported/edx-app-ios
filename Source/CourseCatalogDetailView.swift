    ...
    func setup() {
        ...
        actionButton.oex_addAction({[weak self] _ in
            ...
            self?.action?( {[weak self] _ in
                            ...
                            self?.environment.analytics.trackEvent(OEXAnalytics.enrollEvent(courseId: self?.courseCard.course?.course_id ?? "", name: AnalyticsEventName.CourseEnrollmentClicked.rawValue, displayName: AnalyticsDisplayName.EnrolledCourseClicked.rawValue), forComponent: nil, withInfo: [:])
            } )
            }, for: .touchUpInside)
