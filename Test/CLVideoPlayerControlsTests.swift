...
class CLVideoPlayerControlsTests: XCTestCase {
    ...
    func testValidCaptions() {
        ...
        let expectation = expectationWithDescription("Captioning validation")
        
        let control = CLVideoPlayerControls()
        control.readClosedCaptioningString(TranscriptFactory.validTranscriptString, completion: { (success: Bool) in
    ...
    func testPartialCaptions() {
        ...
        let control = CLVideoPlayerControls()
        control.readClosedCaptioningString(TranscriptFactory.partialTranscriptString, completion: { (success: Bool) in
            XCTAssertTrue(success)
    ...
    func testInvalidCaptions() {
        ...
        let control = CLVideoPlayerControls()
        control.readClosedCaptioningString(TranscriptFactory.invalidTranscriptString, completion: { (success: Bool) in
            XCTAssertTrue(success)
    ...
    func testEmptyCaptions() {
        ...
        let control = CLVideoPlayerControls()
        control.readClosedCaptioningString(TranscriptFactory.emptyTranscriptString, completion: { (success: Bool) in
            XCTAssertTrue(success)
            XCTAssertEqual(control.subtitlesParts().count, 0)
            expectation.fulfill()
        }) { (error) in
