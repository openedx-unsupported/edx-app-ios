...
class DiscussionNewCommentViewControllerTests: SnapshotTestCase {
    ...
    func testContentPost() {
        ...
        let thread = DiscussionThread(
            ...
            responseCount: 0)
        let controller = DiscussionNewCommentViewController(environment: environment, courseID: courseID, threadTitle: thread.title!, context : .Thread(thread))
    ...
    func testContentResponse() {
        ...
        let controller = DiscussionNewCommentViewController(environment: environment, courseID: courseID, threadTitle: "title", context: .Comment(comment))
        inScreenNavigationContext(controller, action: {
            assertSnapshotValidWithContent(controller.navigationController!)
        })
    }
