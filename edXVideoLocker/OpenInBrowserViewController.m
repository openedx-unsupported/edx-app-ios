...
@interface OpenInBrowserViewController ()
@end

static OpenInBrowserViewController * _sharedInterface = nil;

@implementation OpenInBrowserViewController

+ (id)sharedInstance
{
    if (!_sharedInterface) {
        _sharedInterface = [[OpenInBrowserViewController alloc] init];
    }
...
- (void)viewDidLoad
{
    ...
    self.lbl_Title.text = NSLocalizedString(@"OPEN_IN_BROWSER_TITLE", nil);

    
    [self.btn_OpenInBrowser setTitle:NSLocalizedString(@"OPEN_IN_BROWSER", nil) forState:UIControlStateNormal];
...
- (IBAction)openInBrowserClicked:(id)sender
{
    ...
    {
        [Analytics trackOpenInBrowser:_str_browserURL];
