...
- (void)createDatabaseDirectory {
    //File URL
    NSString *basePath = [FileUtility userDirectory];
...
- (void)insertResourceDataForURL:(NSString *)resourceDownloadURL
{
    ...
    resourceObj.resourceFilePath=[FileUtility completeFilePathForUrl:resourceDownloadURL];
    [self saveCurrentStateToDB];
...
- (void)completedDownloadForResourceURL:(NSString *)URL
{
      ResourceData *resourceData = [self resourceDataForURL:URL];
        if (resourceData)
        {
            resourceData.downloadState = [NSNumber numberWithFloat: DownloadStateComplete];
...
- (void)startedDownloadForResourceURL:(NSString *)URL
{
    ResourceData *resourceData = [self resourceDataForURL:URL];
    if (resourceData){
        resourceData.downloadState = [NSNumber numberWithFloat: DownloadStatePartial];
        resourceData.resourceFilePath=[FileUtility completeFilePathForUrl:URL];
        //[self saveCurrentStateToDB];
    }else{
...
- (void)cancelledResourceDownloadForURL:(NSString *)url {
    ...
    //Download State
    newVideoData.downloadState = [NSNumber numberWithInt: DownloadStateNew];
    [self saveCurrentStateToDB];
}
...
-(void)deleteResourceDataForURL:(NSString *)url {
    ...
    NSString * filePath = [FileUtility completeFilePathForUrl:url];
    if (dataToDelete) {
        //Mark new in DB
...
- (DownloadState)downloadStateForResourceURL:(NSString *)URL
{
    ResourceData *resourceData = [self resourceDataForURL:URL];
...
- (void)startedDownloadForURL:(NSString *)downloadUrl andVideoId:(NSString *)videoId
{
    ...
    //Download State
    newVideoData.download_state = [NSNumber numberWithInt: DownloadStatePartial];
    
    [self saveCurrentStateToDB];
}


- (DownloadState)videoStateForVideoID:(NSString *)video_id
{
    ...
    return DownloadStateNew;
}


- (PlayedState)watchedStateForVideoID:(NSString *)video_id
{
    VideoData * videoData = [self getVideoDataForVideoID:video_id];
...
- (void)markPlayedState:(PlayedState)state forVideoID:(NSString *)video_id
{
    VideoData * videoData = [self getVideoDataForVideoID:video_id];
...
- (void)markDownloadState:(DownloadState)Dstate forVideoID:(NSString *)video_id
{
    VideoData * videoData = [self getVideoDataForVideoID:video_id];
...
- (NSData *)resumeDataForVideoID:(NSString *)video_id
{
    ...
    data = [[NSFileManager defaultManager] contentsAtPath:[FileUtility completeFilePathForUrl:videoData.video_url]];
...
- (void)startedDownloadForVideo:(VideoData *)videoData
{
    if(videoData)
    {
        videoData.download_state = [NSNumber numberWithFloat:DownloadStatePartial];
        [self saveCurrentStateToDB];
    }
...
- (void)onlineEntryForVideo:(VideoData *)videoData
{
    if(videoData)
    {
        videoData.download_state = [NSNumber numberWithFloat:DownloadStateNew];
        [self saveCurrentStateToDB];
    }
...
- (void)completedDownloadForVideo:(VideoData *)videoData
{
    if(videoData)
    {
        videoData.download_state = [NSNumber numberWithFloat:DownloadStateComplete];
...
-(void)pausedAllDownloads{
    ...
    for (VideoData *video in array) {
        if([video.download_state intValue]==DownloadStatePartial || [video.dm_id intValue]!=0){
...
- (void)deleteDataForVideoID:(NSString *)video_id
{
    ...
    if([arrVideo count]>1){
        for (VideoData *video in arrVideo) {
            if([video.download_state intValue]==DownloadStateComplete){
                referenceCount++;
                if(referenceCount >=2){
                    break;
                }
    ...
    {
        //Mark new in DB
        videoData.download_state=DownloadStateNew;
        videoData.dm_id=[NSNumber numberWithInt:0];
        [self saveCurrentStateToDB];
        //Delete file
        if(referenceCount <=1)
...
{
    [[NSFileManager defaultManager] removeItemAtPath:[FileUtility localFilePathForVideoUrl:URL]
                                               error:nil];
    
    [[NSFileManager defaultManager] removeItemAtPath:[FileUtility completeFilePathForUrl:URL]
                                               error:nil];
}
...
-(NSArray *)getVideosForDownloadState:(DownloadState)state
{
...
-(NSArray *)getAllDownloadingVideosForURL:(NSString *)url{
    ...
    NSMutableArray *filteredArray=[[NSMutableArray alloc] init];
    for (VideoData *data in allVideos) {
        if([data.download_state intValue]==DownloadStatePartial || [data.dm_id intValue]!=0){
            [filteredArray addObject:data];
        }
    }
    return filteredArray;
...
- (VideoData *)insertVideoData: (NSString *)username
          ...
          DownloadState: (int)download_state
                      ...
                      WithVideoURL: (NSString *)video_url
{
    ...
    {
        VideoData *videoObj = [resultArray objectAtIndex:0];
        videoObj.filepath = [FileUtility relativePathForUrl:video_url];
        [self saveCurrentStateToDB];
    }
}
