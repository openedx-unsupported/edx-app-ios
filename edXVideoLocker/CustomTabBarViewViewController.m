//
//  CustomTabBarViewViewController.m
//  edXVideoLocker
//
...
#import "EDXStyles.h"
#import "FlowErrorViewController.h"
...
#import "TabBarItemsCell.h"
#import "UserDetails.h"

@interface CustomTabBarViewViewController ()<UITableViewDelegate,UITableViewDataSource>
{
...
@property (nonatomic,strong)  HelperVideoDownload *lastAccessedVideo;
@property (nonatomic, strong) NSString *OpenInBrowser_URL;
//for offline
@property (nonatomic, strong) NSMutableArray *arr_OfflineCourseAndVideo;
@property (strong , nonatomic) EDXAnnouncementsView *announcementsView;
...
@implementation CustomTabBarViewViewController
...
- (void)updateCourseWareData
{
    ...
    DataParser *objparser = [[DataParser alloc] initWithDataInterface:_dataInterface];
...
- (void)populateOfflineCheckData
{
    ...
    NSMutableArray *arr_Videos = [[NSMutableArray alloc] initWithArray: [_dataInterface coursesAndVideosForDownloadState:DownloadStateComplete] ];
...
- (void)checkOfflineDataToDisplay:(NSMutableArray *)arr_Data
{
    ...
    for (HelperVideoDownload *objVideos in arr_Data)
    {
...
- (void)viewDidLoad
{
    ...
    self.announcementsView = [[EDXAnnouncementsView alloc] initWithFrame:CGRectMake(0, 108, self.view.frame.size.width, self.view.frame.size.height-108)];
...
{
    __weak CustomTabBarViewViewController *weakself=self;
...
-(void)setNavigationBar
{
    ...
    [Analytics screenViewsTracking:appD.str_NAVTITLE];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    TabBarItemsCell * cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"tabCell" forIndexPath:indexPath];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.row)
    {
        case 0:
            ...
            //Analytics Screen record
            [Analytics screenViewsTracking:[NSString stringWithFormat:@"%@ - Courseware",appD.str_NAVTITLE]];
        ...
        case 1:{
            ...
            //Analytics Screen record
            [Analytics screenViewsTracking:[NSString stringWithFormat:@"%@ - Announcements",appD.str_NAVTITLE]];
        ...
        case 2:
            ...
            //Analytics Screen record
            [Analytics screenViewsTracking:[NSString stringWithFormat:@"%@ - Handouts",appD.str_NAVTITLE]];
...
-(void)loadHandouts
{
    ...
    if (self.html_Handouts.length > 0)
    {
        ...
        NSString* styledHandouts = [EDXStyles styleHTMLContent:self.html_Handouts];
...
- (void)loadAnnouncement
{
    ...
    for(NSDictionary* dict in self.arr_AnnouncementData) {
        [announcements addObject:[[EDXAnnouncement alloc] initWithDictionary: dict]];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([indexPath section] == 0)
    {
        ...
        LastAccessedTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier forIndexPath:indexPath];
        
        HelperVideoDownload *video=self.lastAccessedVideo;
    ...
    else
    {
        ...
        CourseDetailTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier forIndexPath:indexPath];
        ...
        }else
        {
            ...
            else
            {
                if (!_dataInterface.reachable)
                {
                    ...
                    //MOB - 388
                    [[StatusMessageViewController sharedInstance] showMessage:NSLocalizedString(@"SECTION_UNAVAILABLE_OFFLINE", nil)
            ...
            if (_dataInterface.reachable)
            {
                GenericCourseTableViewController *objGeneric = [storyboard instantiateViewControllerWithIdentifier:@"GenericTableView"];
...
-(void)startDownloadChapterVideos:(id)sender
{
    ...
    {
        [Analytics trackSectionBulkVideoDownload: [self.arr_CourseStructureData objectAtIndex:tagValue]
    ...
    if (downloadingCount > 0) {
            [[StatusMessageViewController sharedInstance] showMessage:[NSString stringWithFormat:@"%@ %d %@%@", NSLocalizedString(@"DOWNLOADING", nil),(int)downloadingCount, NSLocalizedString(@"VIDEO", nil), sString]
                                                     onViewController:self.view
                                                             messageY:108
                                                           components:@[self.customNavView , self.tabView, self.customProgressBar, self.btn_Downloads]
                                                           shouldHide:YES];
