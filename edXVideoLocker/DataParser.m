...
- (id)initWithDataInterface:(EdXInterface *)dataInterface
{
    self = [super init];
...
-(id)getHandouts:(NSData *)receivedData{
    ...
    if ([dict isKindOfClass:[NSDictionary class]]) {
        dictResponse=[dict  dictionaryByReplacingNullsWithStrings];
    }
...
- (id)getUserDetails:(NSData *)receivedData
{
    ...
    NSDictionary *dictResponse = [dict  dictionaryByReplacingNullsWithStrings];
    
    UserDetails *obj_userdetails = [[UserDetails alloc] init];
...
//    NSLog(@"\n\n\n\ngetUserCourseEnrollmentList RESPONSE : %@", response);
    ...
    for (NSDictionary *dict in arrResponse)
    {
        ...
        UserCourseEnrollment *obj_usercourse = [[UserCourseEnrollment alloc] init];
        ...
        Course *obj_Course = [[Course alloc] init];
        ...
        LatestUpdates *obj_LatestUpdate = [[LatestUpdates alloc] init];
...
//    NSLog(@"\n\n\n\ngetVideoSummaryList Response \n %@ ",response);
    ...
    for (NSDictionary *dict in arrResponse)
    {
        ...
        VideoSummaryList *objvideosummary =[[VideoSummaryList alloc ] init];
...
- (NSString *)getOpenInBrowserLink
{
    ...
    for (VideoSummaryList *objVideo in [appD.dict_VideoSummary objectForKey:appD.str_COURSE_OUTLINE_URL])
    {
...
- (id)getLevel1DataForURLString:(NSString *)URL
{
    ...
    for (VideoSummaryList *objVideo in [appD.dict_VideoSummary objectForKey:URL])
    {
...
- (NSMutableArray *)getVideosOfCourseWithURLString:(NSString *)URL
{
    ...
    for (VideoSummaryList *objVideo in [appD.dict_VideoSummary objectForKey:URL])
    {
        HelperVideoDownload *obj_helperVideo = [[HelperVideoDownload alloc] init];
        ...
        obj_helperVideo.filePath=[FileUtility completeFilePathForUrl:obj_helperVideo.str_VideoURL];
