...
@property (weak, nonatomic) IBOutlet CustomButton *btn_OpenEULA;
@property (weak, nonatomic) IBOutlet UIImageView *img_SeparatorEULA;
@property (strong, nonatomic) UIImageView *imgOverlay;
@property (weak, nonatomic) IBOutlet CustomButton *btn_Facebook;
@property (weak, nonatomic) IBOutlet CustomButton *btn_Google;
@property (weak, nonatomic) IBOutlet CustomLabel *lbl_OrSignIn;
...
- (void)viewDidLoad
{
    ...
    [Analytics screenViewsTracking:@"Login"];
...
- (void)reachabilityDidChange:(NSNotification *)notification {
    ...
    } else
    {
        ...
        [[FlowErrorViewController sharedInstance] showErrorWithTitle:NSLocalizedString(@"NETWORK_NOT_AVAILABLE_TITLE", nil)
...
- (IBAction)signUpClicked:(id)sender
{
    ...
    [Analytics trackUserDoesNotHaveAccount];
...
- (IBAction)loginClicked:(id)sender
{
    ...
    if ([self.tf_EmailID.text length] == 0)
    {
        [[FlowErrorViewController sharedInstance] showErrorWithTitle:NSLocalizedString(@"FLOATING_ERROR_LOGIN_TITLE", nil)
    ...
    else
    {
        ...
        [EdxAuthentication requestTokenWithUser:_signInID
...
-(void)handleLoginResponseWith:(NSData *)data response:(NSURLResponse *)response error:(NSError *)error{
    ...
    [[GoogleSocial sharedInstance]clearHandler];
    [[FBSocial sharedInstance]clearHandler];
...
- (IBAction)facebookClicked:(id)sender
{
    ...
    [self.view setUserInteractionEnabled:NO];
     [self socialLoginWith:FACEBOOK_LOGIN];
}
...
- (IBAction)googleClicked:(id)sender
{
    isSocialLoginClicked=YES;
    [self.view setUserInteractionEnabled:NO];
    [self socialLoginWith:GOOGLE_LOGIN];
}


-(void)socialLoginWith:(SocialLoginType)type{
...
//#warning solve MOB-1115 here.
    ...
    [EdxAuthentication socialLoginWith:type completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        if (!response) {
            [self performSelectorOnMainThread:@selector(loginFailed:) withObject:NSLocalizedString(@"INVALID_USERNAME_PASSWORD", nil ) waitUntilDone:NO ];
            return ;
        }
...
- (void)loginSuccessful {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if([_tf_EmailID.text length]>0)
        {
            ...
            [Analytics trackUserLogin:self.strLoggedInWith];
        }
    });
    
    [self tappedToDismiss];
    
    UserDetails *objUser = [EdxAuthentication getLoggedInUser];
    if(objUser){
    [[EdXInterface sharedInterface] loggedInUser:objUser];
    [[EdXInterface sharedInterface] activateIntefaceForUser:objUser];
    [Analytics identifyUser:[NSString stringWithFormat:@"%ld",objUser.User_id] Email:objUser.email Username:objUser.username];
    //Init background downloads
    [[EdXInterface sharedInterface] startAllBackgroundDownloads];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if (alertView.tag == 1001)
    {
        ...
        if (buttonIndex == 1)
        {
            if ([EmailtextField.text length]==0 || ![AppDelegate isEmailValid:EmailtextField.text])
            {
                [[FlowErrorViewController sharedInstance] showErrorWithTitle:NSLocalizedString(@"FLOATING_ERROR_TITLE", nil)
            ...
            else
            {
                ...
                [[FlowErrorViewController sharedInstance] showErrorWithTitle:NSLocalizedString(@"RESET_PASSWORD_TITLE", nil)
...
- (void)resetPassword
{
    [EdxAuthentication resetPasswordWithEmailId:self.str_ForgotEmail CSRFToken:self.str_CSRFToken completionHandler:^(NSData *data, NSURLResponse *response, NSError *error)
     {
         ...
         if (!error)
         {
             ...
             dispatch_async(dispatch_get_main_queue(), ^{
                 ...
                 if (httpResp.statusCode == 200)
                 {
                     ...
                     [[FlowErrorViewController sharedInstance] showErrorWithTitle:NSLocalizedString(@"FLOATING_ERROR_TITLE", nil)message:responseStr onViewController:self.view shouldHide:YES];
             ...
             [[FlowErrorViewController sharedInstance] showErrorWithTitle:NSLocalizedString(@"FLOATING_ERROR_TITLE", nil) message:[error localizedDescription] onViewController:self.view shouldHide:YES];
