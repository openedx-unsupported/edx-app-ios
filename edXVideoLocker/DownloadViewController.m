...
@property(strong,nonatomic)EdXInterface *edxInterface;
@property(strong,nonatomic)IBOutlet NSLayoutConstraint *recentDownloadViewHeight;
@property(strong,nonatomic)IBOutlet CustomLabel *lbl_downloadedCount;
@property(strong,nonatomic)IBOutlet CustomLabel *lbl_DownloadedText;
...
- (void)viewDidLoad
{
    ...
    _edxInterface=[EdXInterface sharedInterface];
...
-(void)reloadDownloadingVideos
{
    ...
    NSMutableArray *array = [_edxInterface coursesAndVideosForDownloadState:DownloadStatePartial];
...
{
    DownloadTableCell *cell = [tableView dequeueReusableCellWithIdentifier:@"CellDownloads" forIndexPath:indexPath];
...
-(void)configureCell:(DownloadTableCell *)cell forIndexPath:(NSIndexPath *)indexPath{
    
    if([self.arr_downloadingVideo count] > indexPath.row)
    {
        
        HelperVideoDownload *downloadingVideo= [self.arr_downloadingVideo objectAtIndex:indexPath.row];
        ...
        if (!downloadingVideo.duration)
            ...
            cell.lbl_time.text = [AppDelegate timeFormatted: [NSString stringWithFormat:@"%.1f", downloadingVideo.duration]];
...
-(void)tableView:(UITableView *)tableView willDisplayCell:(DownloadTableCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath{
...
-(void)downloadProgressNotification:(NSNotification *)notification
{
    ...
    for (HelperVideoDownload * video in _arr_downloadingVideo) {
...
-(void)updateProgressForVisibleRows{
    ...
    if(![self.table_Downloads isDecelerating] ||  ![self.table_Downloads isDragging]){
        ...
        for (DownloadTableCell *cell in array)
        {
            
            NSIndexPath *indexPath = [self.table_Downloads indexPathForCell:cell];
            HelperVideoDownload *video=[self.arr_downloadingVideo objectAtIndex:indexPath.row];
...
-(IBAction)btnCancelPressed:(UIButton *)button{
    ...
    EdXInterface *edxInterface=[EdXInterface sharedInterface];
