//
//  CourseVideoDownloadTableViewController.m
//  edXVideoLocker
//
...
@interface CourseVideoDownloadTableViewController ()<videoPlayerInterfaceDelegate,UITableViewDataSource,UITableViewDelegate>
@property (nonatomic, strong) EdXInterface * dataInterface;
@property (nonatomic, strong) VideoPlayerInterface * videoPlayerInterface;
@property (nonatomic, strong) HelperVideoDownload * currentTappedVideo;
...
@implementation CourseVideoDownloadTableViewController
...
-(void)viewDidLoad
{
    ...
    self.obj_DataParser = [[DataParser alloc] initWithDataInterface:_dataInterface];
    ...
    {
        HelperVideoDownload *video = [self.arr_DownloadProgress firstObject];
        [_dataInterface updateLastVisitedModule:video.subSectionID];
    }
    
    //Analytics Screen record
    [Analytics screenViewsTracking:@"My Courses"];
...
- (void)removeDuplicateData
{
    ...
    for (NSArray *array in arrTemp)
    {
        ...
        for (HelperVideoDownload *videos in array)
        {
            // Comparing objects
            for (NSArray *arrayCompare in arrTemp)
            {
                for (HelperVideoDownload *videosCompare in arrayCompare)
                {
...
- (void)checkAndchangeDownloadStateOfVideos:(NSMutableArray *)arrayVideos
{
    ...
    {
        for (HelperVideoDownload *videos in [dict objectForKey:CAV_KEY_VIDEOS])
        {
    ...
    // If yes, then change the Downloadstate to complete in arr_SibsectionData
    
    BOOL isVideoAvail = NO;
    
    for (NSMutableArray *arr in self.arr_SubsectionData)
    {
        for (HelperVideoDownload *obj in arr)
        {
            ...
            for (HelperVideoDownload *comparevideo in temp)
            {
                ...
                {
                    isVideoAvail = YES;
                    obj.state = DownloadStateComplete;
...
-(void)showDeviceGoneOfflineMessage
{
    ...
    if(self.playbackViewHeightConstraint.constant >0 && self.currentTappedVideo.state!=DownloadStateComplete )// To
    {
        
        [[StatusMessageViewController sharedInstance] showMessage:NSLocalizedString(@"CHECK_CONNECTION", nil)
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else if (_dataInterface.reachable && !_isFromGenericView) /// online mode
    {
        ...
        CourseVideosTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIndentifier forIndexPath:indexPath];
    ...
    else
    {
        ...
        CourseVideosTableViewCell *cell = [self.table_Videos dequeueReusableCellWithIdentifier:@"CellCourseVideo" forIndexPath:indexPath];
        ...
        HelperVideoDownload *obj = [self.arr_DownloadProgress objectAtIndex:indexPath.row];
        //Progress
        cell.customProgressView.tag = indexPath.row;
        ...
        if (!obj.duration)
            ...
            cell.lbl_Time.text = [AppDelegate timeFormatted: [NSString stringWithFormat:@"%.1f", obj.duration]];
        ...
        //Played state
        UIImage * playedImage;
        if (obj.watchedState == PlayedStateWatched) {
            playedImage = [UIImage imageNamed:@"ic_watched.png"];
        }
        else if (obj.watchedState == PlayedStatePartiallyWatched) {
            playedImage = [UIImage imageNamed:@"ic_partiallywatched.png"];
        }
        else {
            playedImage = [UIImage imageNamed:@"ic_unwatched.png"];
...
-(UITableViewCell *)configureOfflineCellFor:(UITableView *)tableView indexPath:(NSIndexPath *)indexPath
{
    ...
    if (obj_video.state == DownloadStateComplete)
    {
        cell.backgroundColor = [UIColor whiteColor];
...
{    
    ...
    if (_dataInterface.reachable)
    {
        ...
        if (obj.state == DownloadStateComplete)
        ...
        else
        {
            ...
            }else
            {
                if(obj.str_VideoURL==nil || [obj.str_VideoURL isEqualToString:@""]){
                    ...
                    [[StatusMessageViewController sharedInstance] showMessage:NSLocalizedString(@"VIDEO_CONTENT_NOT_AVAILABLE", nil)
                                                             onViewController:self.view
                                                                     messageY:64
    ...
    else
    {
        ...
        {
            [[StatusMessageViewController sharedInstance] showMessage:NSLocalizedString(@"VIDEO_NOT_AVAILABLE", nil)
                                                     onViewController:self.view
                                                             messageY:64
                                                           components:@[self.customNavView, self.customProgressBarTotal, self.btn_SelectAllEditing, self.btn_Downloads]
                                                           shouldHide:YES];
...
- (void)playVideoFromLocal:(HelperVideoDownload *)obj
{
    [self handleComponentsFrame];
    //stop current video
    [_videoPlayerInterface.moviePlayerController stop];
...
- (void)startDownloadVideo:(id)sender
{
    ...
    HelperVideoDownload *obj = [self.arr_DownloadProgress objectAtIndex:tagValue];
    
    
    // Analytics Single Video Download
    if (obj.video_id)
    {
        [Analytics trackSingleVideoDownload: obj.video_id
...
__weak id weakself=self;
   ...
   [_dataInterface startDownloadForVideo:obj completionHandler:^(BOOL success){
     dispatch_async(dispatch_get_main_queue(), ^{
         ...
         [[StatusMessageViewController sharedInstance] showMessage:NSLocalizedString(@"DOWNLOADING_1_VIDEO", nil)
                                                  onViewController:self.view
                                                          messageY:64
                                                        components:@[self.customNavView, self.customProgressBarTotal, self.btn_SelectAllEditing, self.btn_Downloads]
                                                        shouldHide:YES];
         if(obj.DownloadProgress==100){
             CourseVideosTableViewCell *cell=(CourseVideosTableViewCell *)[self.table_Videos cellForRowAtIndexPath:[NSIndexPath indexPathForRow:tagValue inSection:0]];
             cell.customProgressView.hidden=NO;
             cell.btn_Download.hidden=YES;
...
-(void)deleteSelectedVideos{
    ...
    for (NSArray *arr in self.arr_SubsectionData)
    {
        for (HelperVideoDownload *videos in arr)
        {
            if (videos.state == DownloadStateComplete)
            {
...
-(BOOL)checkIfVideoFileDownloaded:(HelperVideoDownload *)video{
    
    NSString *fileUrl=[FileUtility localFilePathForVideoUrl:video.str_VideoURL];
    ...
    if(! _dataInterface.reachable){
        ...
        if(indexPath!=nil){
            ...
            if([self.arr_SubsectionData count] > indexPath.section){
                ...
                if([[self.arr_SubsectionData objectAtIndex:indexPath.section] count] > indexPath.row){
                    ...
                    HelperVideoDownload *video=[[self.arr_SubsectionData  objectAtIndex:indexPath.section]  objectAtIndex:indexPath.row];
                    if(video)
                    {
                        return indexPath;
                    }
        ...
        if(indexPath!=nil){
            ...
            if([self.arr_DownloadProgress count] > indexPath.row){
                ...
                HelperVideoDownload *video=[self.arr_DownloadProgress  objectAtIndex:indexPath.row];
                if(video){
                    return indexPath;
                }
...
- (void)playbackEnded:(NSNotification *)notification
{
    ...
    if (reason == MPMovieFinishReasonPlaybackEnded) {
        ...
        if (_dataInterface.reachable)
        {
            if(_currentTappedVideo.state == DownloadStateNew || _currentTappedVideo.state == DownloadStatePartial)
            {
...
- (void)downloadProgressNotification:(NSNotification *)notification
{
    ...
    for (HelperVideoDownload * video in _arr_DownloadProgress) {
