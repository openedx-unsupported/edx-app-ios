...
@property(nonatomic,strong)HelperVideoDownload *currentVideo;
@property(nonatomic,strong)HelperVideoDownload *lastPlayedVideo;
...
- (void)playVideoFor:(HelperVideoDownload *)video
{
    ...
    if(video.state==DownloadStateComplete && ![filemgr fileExistsAtPath:slink]){
        return;
    }
    
    float timeinterval=[[EdXInterface sharedInterface] lastPlayedIntervalForVideo:video];
...
-(void)updateLastPlayedVideoWith:(HelperVideoDownload *)video{
    if(_currentVideo){
...
-(void)playerDidStopPlaying:(NSURL *)videoUrl atPlayBackTime:(float)timeinterval{
    ...
    if([_lastPlayedVideo.str_VideoURL isEqualToString:url] || [_lastPlayedVideo.filePath isEqualToString:url]){
        if(timeinterval > 0 ){
            [[EdXInterface sharedInterface] markLastPlayedInterval:timeinterval forVideo:_lastPlayedVideo];
        }
    }else{
        if(timeinterval > 0 ){
            [[EdXInterface sharedInterface] markLastPlayedInterval:timeinterval forVideo:_currentVideo];
        }
    }
